...
void chip8_cycle(chip8_t *chip8)
{
    ...
    case 0xD: { // DXYN: Draw sprite
        // Extract operands
        const uint8_t vx = chip8->V[(chip8->opcode & 0x0F00) >> 8];
        const uint8_t vy = chip8->V[(chip8->opcode & 0x00F0) >> 4];
        ...
        for (uint8_t row = 0; row < height; row++)
        {
            ...
            {
                // Check if the current bit is set in sprite byte
                if ((sprite_byte & (0x80 >> col)) != 0)
                {
                    uint8_t x_pos = (vx + col) % 64;
                    uint8_t y_pos = (vy + row) % 32;
                    uint16_t idx = x_pos + (y_pos * 64);
